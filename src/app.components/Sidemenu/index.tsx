...
export default function SideMenu({ handleClickOfSideMenu }: Props) {
	...
	const sideNavigates = [
		{ text: '목표 인증', to: SERVICE_URL.certifications },
		{ text: '목표머니 충전', to: SERVICE_URL.moneyCharge },
		{ text: '목표머니 이체', to: SERVICE_URL.moneyTransfer },
		{ text: '내 목표', to: SERVICE_URL.myGoals },
		{ text: '', to: '' },
		{ text: '공지사항', to: SERVICE_URL.announcements },
		{ text: '설정', to: SERVICE_URL.setting },
	];
			...
			<aside className="absolute top-0 right-0 h-full overflow-auto w-[265px] bg-primaryWhite">
				<div className="p-[20px] flex flex-col">
					<div className="basis-[72px] flex justify-between items-center">
						...
						<button type="button" onClick={handleClickOfSideMenu}>
							<Link to={SERVICE_URL.pushNotice}>{/*<Alarm /> */}</Link>
						</button>
					</div>
					<div className="basis-[109px] flex flex-col items-center space-y-[20px]">
						...
						>
							<Link className="side-menu-user-name" to={true ? SERVICE_URL.myGoals : SERVICE_URL.login}>
								{true ? '닉네임' : '로그인 해주세요'}
							</Link>
						</button>
